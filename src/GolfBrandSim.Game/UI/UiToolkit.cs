...
public static class UiToolkit
{
    public static void DrawPanel(UiContext ui, Rectangle bounds, string title)
    {
        ...
        var titleBounds = new Rectangle(bounds.X, bounds.Y, bounds.Width, 36);
        ui.FillRectangle(titleBounds, Theme.PanelRaised);
        ui.DrawBorder(titleBounds, Theme.PanelBorder, 2);
        ui.DrawText(title, new Vector2(bounds.X + 14, bounds.Y + 10), Theme.TextPrimary, 2);
    }
    ...
    public static void DrawSummaryCard(UiContext ui, Rectangle bounds, string label, string value, string footer)
    {
        ...
        ui.DrawText(label, new Vector2(bounds.X + 14, bounds.Y + 12), Theme.TextMuted, 2);
        ui.DrawText(value, new Vector2(bounds.X + 14, bounds.Y + 44), Theme.TextPrimary, 3);
        ui.DrawText(footer, new Vector2(bounds.X + 14, bounds.Bottom - 24), Theme.TextMuted, 2);
    }
        ...
        int highlightedRowIndex = -1)
    {
        ...
        for (var index = 0; index < headers.Count; index++)
        {
            ...
            ui.DrawText(headers[index], new Vector2(cellBounds.X + 8, cellBounds.Y + 10), Theme.TextPrimary, 2);
            cursorX += width;
        }
        ...
        for (var rowIndex = 0; rowIndex < Math.Min(rows.Count, maxRows); rowIndex++)
        {
            ...
            for (var columnIndex = 0; columnIndex < headers.Count; columnIndex++)
            {
                ...
                ui.DrawText(value, new Vector2(cellBounds.X + 8, cellBounds.Y + 8), Theme.TextPrimary, 2);
