...
public sealed class FinanceScreen : IScreen
{
    ...
    public void Draw(UiContext ui, GameSession session, Rectangle bounds)
    {
        ...
        UiToolkit.DrawSummaryCard(ui, new Rectangle(bounds.X, bounds.Y, 250, 120), "CASH", Formatters.Money(session.State.PlayerBrand.CashBalance), "CURRENT BALANCE");
        UiToolkit.DrawSummaryCard(ui, new Rectangle(bounds.X + 270, bounds.Y, 250, 120), "PRODUCTS", Formatters.Money(productRevenue), "SEASON TO DATE");
        UiToolkit.DrawSummaryCard(ui, new Rectangle(bounds.X + 540, bounds.Y, 250, 120), "SPONSORSHIP", Formatters.Money(sponsorshipRevenue), "SEASON TO DATE");
        UiToolkit.DrawSummaryCard(ui, new Rectangle(bounds.X + 810, bounds.Y, 250, 120), "RETAINERS", Formatters.Money(operatingCost), "CASH OUT");
        UiToolkit.DrawSummaryCard(ui, new Rectangle(bounds.X + 1080, bounds.Y, 250, 120), "LAST WEEK", session.State.LastWeekResult is null ? "NO DATA" : Formatters.Money(session.State.LastWeekResult.NetCashChange), "NET CHANGE");

        UiToolkit.DrawPanel(ui, new Rectangle(bounds.X, bounds.Y + 150, bounds.Width, bounds.Height - 150), "LEDGER");
        ...
        UiToolkit.DrawTable(
            ui,
            new Rectangle(bounds.X + 16, bounds.Y + 202, bounds.Width - 32, bounds.Height - 218),
